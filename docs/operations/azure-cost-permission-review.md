...
- ACR, ACA, PostgreSQL, Azure Managed Redis, CDN/Front Door, Event Hubs는 skeleton/plan-only 이후 별도 apply 승인 전에 budget impact를 확인한다.
...
| Azure Managed Redis | region, SKU/tier, 운영 시간 |
...
| Azure Managed Redis | 중간 | cache는 상시 기동 리소스라 idle 비용이 남고, access key 기반 운영 시 state 접근 통제와 key rotation 절차가 필요하다 |
...
| Medium | PostgreSQL B1ms, Azure Managed Redis, ACR Basic | 운영 시간과 SKU 재검토 |
