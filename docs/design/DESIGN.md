...
- 시작 전 약속에는 정산 CTA를 노출하지 않는다.
- 정산 만들기는 긴 입력 폼보다 결제 항목 카드 리스트와 하단 CTA를 우선한다.
- 결제 항목은 방문장소별 section과 `기타 비용` section으로 묶어 표시한다.
- `참여자별`보다 `개별 금액`, `대상자`처럼 사용자가 바로 이해하는 말을 쓴다.
- 결과 화면은 총액 카드가 과하게 커지지 않게 압축형 요약으로 보여준다.
- 결과 화면의 participant grid는 한 줄 최대 4명까지 배치한다.
- 수취자는 프로필 위쪽에 작은 왕관 아이콘을, 완료된 사용자는 초록 체크 아이콘을 overlay한다.
- 송금/수취 완료 버튼은 사용자의 현재 역할에 맞는 하나의 primary action으로 둔다.
- `정산 근거`는 결과 화면 하단의 작은 secondary action으로 제공한다.
