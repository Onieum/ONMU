...
발표용 다이어그램과 기술 스택 선택 기준은 [현재 아키텍처 다이어그램과 기술 스택 권장안](./current-architecture-diagram.md)을 기준으로 함께 관리합니다. Flutter 구조는 [Flutter 프론트 아키텍처](./frontend-architecture.md), API 계약은 [API Contract Map](./api-contract-map.md), 백엔드 선택지는 [백엔드 기술스택 선택지](./backend-stack-options.md)를 함께 봅니다.
...
| 워커 | 추천 설명, 장소 선택 보조, 경로/출발, 사진 기록, 알림 |
...
2. 백엔드 Main API 기술스택을 spike 후 확정합니다.
