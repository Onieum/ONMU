...
2. `POST /groups/{groupId}/plans`가 `participantUserIds` 포함 요청을 받고, 생성자와 대상 참여자를 반환한다.
3. `GET /plans/{planId}/participants`가 active participant만 반환한다.
4. 장소 후보를 만든 뒤 `POST /groups/{groupId}/votes`에 `placeCandidateIds`를 넣으면 option object가 후보 정보와 함께 반환된다.
5. `git diff --check`와 문서 링크 확인을 통과한다.
...
| 2 | Flutter 홈을 `GET /home/summary` 소비 구조로 전환 |
| 3 | 투표 응답 API와 Flutter 선택/저장 UX 구현 |
| 4 | 투표 종료와 decision/action card 구현 |
| 5 | outbox -> Service Bus -> Notification/Realtime/Worker 연결 |
| 6 | 홈/약속/투표 Application Insights metric과 smoke 대시보드 구성 |
