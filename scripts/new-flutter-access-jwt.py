...
def default_ios_google_xcconfig_path() -> Path:
    return (
        repo_root()
        / "apps"
        / "mobile-flutter"
        / "ios"
        / "Flutter"
        / "GoogleOAuth.generated.xcconfig"
    )
...
def google_reversed_client_id(client_id: str) -> str:
    ...
    if not trimmed.endswith(suffix):
        raise RuntimeError(
            "GOOGLE_CLIENT_ID must end with .apps.googleusercontent.com to derive the iOS URL scheme."
        )
    return f"com.googleusercontent.apps.{trimmed.removesuffix(suffix)}"


def xcconfig_value(value: str) -> str:
    trimmed = value.strip()
    if any(character.isspace() for character in trimmed):
        raise RuntimeError("OAuth xcconfig values must not contain whitespace.")
    return trimmed


def write_ios_google_xcconfig(
    path: Path,
    client_id: str,
    ...
    body = "\n".join(
        [
            "// Generated by scripts/new-flutter-access-jwt.py.",
            "// Contains public Google OAuth identifiers only. Do not commit this file.",
            f"GOOGLE_IOS_CLIENT_ID = {xcconfig_value(client_id)}",
            f"GOOGLE_IOS_SERVER_CLIENT_ID = {xcconfig_value(server_client_id)}",
            f"GOOGLE_IOS_REVERSED_CLIENT_ID = {xcconfig_value(reversed_client_id)}",
            "",
        ]
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--ios-google-xcconfig-path",
        help="Path for generated iOS Google OAuth xcconfig. Defaults to the ignored Flutter config path.",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    if include_google_oauth:
        try:
            ...
            google_reversed_id = google_reversed_client_id(google_oauth_client_id)
        except RuntimeError as exc:
            print(str(exc), file=sys.stderr)
            return 1
        ...
        if not args.skip_ios_google_xcconfig:
            ios_google_xcconfig_path = (
                Path(args.ios_google_xcconfig_path).expanduser()
                if args.ios_google_xcconfig_path
                else default_ios_google_xcconfig_path()
            ).resolve()
            try:
                write_ios_google_xcconfig(
                    ios_google_xcconfig_path,
