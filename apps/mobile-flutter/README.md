...
Naver는 기존 dev scaffold를 유지하되 최종 앱 세션은 Spring OAuth exchange 응답으로 받은 ONMU token을 기준으로 만듭니다. Google은 이후 idToken 검증 provider를 Spring에 추가할 수 있도록 별도 follow-up으로 둡니다.
