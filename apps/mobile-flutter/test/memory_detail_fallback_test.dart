...
void main() {
  test('memory detail provider falls back to record list when detail is missing', () async {
  ...
  test('daily record edit load falls back to record list when detail is missing', () async {
    final container = ProviderContainer(
      overrides: [
        recordRepositoryProvider.overrideWithValue(
          _DetailMissingRecordRepository(),
  ...
  test('group memory detail falls back to group memory list when detail is missing', () async {
...
class _DetailMissingRecordRepository implements RecordRepository {
  ...
  @override
  Future<List<OotdRecord>> fetchMyRecords() async => List.unmodifiable(_records);
